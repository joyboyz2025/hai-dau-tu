...
7c. **Sổ mã** (`HDT.tickers[]` — trang "Sổ mã" + trang chi tiết từng mã, thêm 19/07): mỗi phần tử `{key, symbol, name, group (Chứng khoán Việt Nam|Quốc tế|Hàng hóa & Crypto), tv (symbol TradingView, rỗng = không chart), sig, stance, oneLiner, aliases[] (chuỗi không dấu để auto-gom "Nhắc đến" + "Bảng điểm của mã" từ ALL_CALLS/scorecard), thesis[{title,body}] (luận điểm dài — CHỈ từ transcript), orders[{expertId,dir,sig,entry,target,stop,status:active|waiting|done,date,note}], potential (1–5) + potentialNote, caution (1–5) + cautionNote (thang Tiềm năng/Thận trọng của KHUYẾN NGHỊ hiện tại — chỉnh lại mỗi khi stance đổi)}`. Hiện 36 mã (VN 15 · Quốc tế 17 · Hàng hóa & Crypto 4); mỗi mã một trang riêng — mã mới được nhắc trong transcript thì tách trang riêng, đừng gộp. Trang danh sách là dạng infographic (dải thống kê + phân bố tín hiệu/nhóm) có nút "Xuất PDF" (window.print + CSS @media print, class no-print/print-only/tk-card đã có sẵn). Mỗi lần update: sửa stance/orders theo transcript mới (lệnh khớp mục tiêu → status done), thêm mã mới khi chuyên gia bắt đầu theo dõi, cập nhật thesis khi có luận điểm mới.
